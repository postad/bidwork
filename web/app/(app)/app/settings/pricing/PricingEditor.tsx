...
function FlooringForm({ label, card, onChange }: { label: string; card: FlooringCard; onChange: (c: FlooringCard) => void }) {
  const [showAddons, setShowAddons] = useState(
    card.prepPerSqft != null || card.baseTrimPerLf != null || card.discountPct != null || card.taxPct != null,
  );
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Global charges — flat $ or % of products (mobilization, delivery, …) */}
      <div className="border-t border-bw-border pt-4">
        <GlobalChargesEditor charges={card.globalCharges ?? []} onChange={(g) => onChange({ ...card, globalCharges: g })} />
      </div>

      {/* Optional add-ons, collapsed by default */}
      <div className="border-t border-bw-border pt-4">
        {!showAddons ? (
          <button type="button" onClick={() => setShowAddons(true)} className="text-[13px] text-bw-green font-semibold">+ Add optional extras (prep, base, discount, tax)</button>
        ) : (
