...
/** Load every covered sub-trade with its current rate card + completeness flag. */
export async function getPricingModel(): Promise<TradeCard[]> {
  ...
  const { data: trades } = await supabase.from("trades").select("id, slug, label, category").in("id", ids);
  ...
  return (trades ?? [])
    .map((t) => {
      ...
      const base = { tradeId: t.id as string, slug: t.slug as string, label: t.label as string, category: (t.category as string) ?? "" };
      if (t.category === "flooring") {
        const { card, complete } = buildFlooring(its);
        return { ...base, flooring: card, complete };
      }
