...
/**
 * The shape the engine emits. The whole point of the spike is to confirm THIS
 * shape survives contact with a real 100+ page bid set — it becomes the basis
 * for the `extractions` JSONB and `bid_line_items` columns in the real schema.
 *
 * Every extracted fact carries a confidence (0..1) and a citation so nothing is
 * ever a bare number. The model is explicitly allowed to return null + a reason
...
export const ExtractionResult = z.object({
  ...
  referencedSheets: z.array(z.string()).describe("Sheets/specs the documents point to (for reference-chasing gap detection), e.g. 'see A-601'"),
});

export type ExtractionResult = z.infer<typeof ExtractionResult>;

/** Convert a zod object schema into a JSON Schema for Anthropic tool input. */
export { z };
