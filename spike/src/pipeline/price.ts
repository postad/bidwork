/**
 * Deterministic pricing — the engine NEVER lets the model do arithmetic.
 * Given a structured scope + the tenant's Pricing DNA, compute line items and
 * totals exactly. This mirrors the proposal's order of operations:
...
export function priceScope(scope: Scope, dna: PricingDNA, discountPct = dna.defaultDiscountPct) {
  ...
  for (const s of scope.motorizedSets) {
    ...
    lines.push({ code: "WT", label: `Motorized roller — ${s.shadesPerMotor} on 1 motor${s.location ? ` (${s.location})` : ""}`, qty: 1, unitRate: rate, amount: rate });
  }
  for (const b of scope.blinds) {
    ...
    lines.push({ code: "MB", label: `Manual aluminum blind${b.widthInches ? ` (${b.widthInches}" W)` : ""}`, qty: 1, unitRate: tier.price, amount: tier.price });
  }
